...
data events_list gluc_list freestyle_list;
	set list;
	if find(path,"events.csv","i") and ^find(path,"BGM","i") and find(path,"2290")  then output events_list;
    if find(path,"gluc.csv","i") or find(path,"glucPlus.csv") and ^find(path,"BGM","i") and find(path,"2290") then output gluc_list;
